...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		...
		if auth, ok := ctx.Value(ContextOauth2AccessToken).(string); ok {
			localVarRequest.Header.Add("Authorization", "Bearer "+auth)
		}

		// Api Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(string); ok {
			localVarRequest.Header.Add("Authorization", "App "+auth)
		}

		// IBSSO Authentication
		if auth, ok := ctx.Value(ContextIBSSO).(string); ok {
			localVarRequest.Header.Add("Authorization", "IBSSO "+auth)
		}
	}
