...
This document defines the versioning strategy for the Infobip Go API Client.
...
* **Correctness takes priority over Go API backward compatibility.**
...
## 4. Go and Dependency Compatibility Policy

### 4.1 Minimum Go Version Changes

Increasing the **minimum supported Go version** (e.g., Go 1.11 → Go 1.18) is always treated as a **MAJOR** version change. Such a change may break compilation and runtime compatibility for existing users and therefore constitutes a breaking API change.

### 4.2 Dependency Upgrade Policy

**PATCH**

* Only patch-level dependency updates are allowed.
...
* Patch or minor dependency updates are allowed.
...
* Major dependency upgrades should be shipped in major releases.
